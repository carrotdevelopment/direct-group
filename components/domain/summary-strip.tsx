      ...
      {items.map((item, index) => {
        ...
        const content = (
          <>
            ...
            <div>
              <div className="text-[10px] font-bold uppercase tracking-[.08em] text-[#89918b]">{item.label}</div>
              <div className="tabular mt-1 text-xl font-black tracking-tight">{item.value}</div>
              <div className="mt-0.5 text-[10px] text-[#879089]">{item.meta}</div>
            </div>
          </>
        );
        ...
        return item.href ? (
          <Link
            ...
            title={`Ir a ${item.label.toLowerCase()}`}
          >
            {content}
          </Link>
        ) : (
